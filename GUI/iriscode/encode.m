...
function [template, mask, phaseTS] = encode(polar_array,noise_array, nscales, minWaveLength, mult, sigmaOnf)
...
for k=1:nscales
    ...
    %@saket : code modified to generate the phase time series
    cmplx_signal = reshape(E1,1,[]);
    noise_mask_ts = reshape(noise_array,1,[]);
    noise_idx = (abs(cmplx_signal)<0.0001) | noise_mask_ts;
    cmplx_signal(noise_idx) = NaN;
    phaseTS = angle(cmplx_signal);
    %%%%%
