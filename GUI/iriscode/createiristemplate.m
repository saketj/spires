...
function [template, mask, phaseTS, image_segmented, image_noise, image_normal] = createiristemplate(eyeimage_filename)
...
%%% added by saket
w = cd;
[pathstr, name, ext, versn] = fileparts(eyeimage_filename);
appDataFolder = 'appData';
eyeimage_filename = fullfile(w,appDataFolder,strcat(name,ext),versn);
%%%
...
%output images
image_noise = imagewithnoise2;
image_segmented = imagewithcircles;

cd(w);

% perform normalisation

[polar_array noise_array image_normal] = normaliseiris(imagewithnoise, circleiris(2),...
...
[template mask phaseTS] = encode(polar_array, noise_array, nscales, minWaveLength, mult, sigmaOnf); 
N = length(phaseTS);
for i = 1:N
    if(isnan(phaseTS(i)))
        phaseTS(i) = 0;
